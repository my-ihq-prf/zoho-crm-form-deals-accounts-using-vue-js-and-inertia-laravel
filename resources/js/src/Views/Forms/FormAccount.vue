...
const onSubmit = (_onSuccess) => {
   ...
   if (props.crud === 'create') {
      ntf = () => consts.notyf.success(`The app has successfully created the account with ID:${props.id}`)
   } else if (props.crud === 'update') {
      ntf = () => consts.notyf.success(`The app has successfully updated the account with ID:${props.id}`)
   } else if (props.crud === 'delete') {
      ntf = () => consts.notyf.success(`The app has successfully deleted the account with ID:${props.id}`)
   }
   const d = {
      ...form
   }
   ...
   useAccounts()[props.crud](d).then((/* ...args */) => {
...
const acc = store.mainAccounts.find(_acc => _acc.id === props.id)
if (acc) {
   Object.assign(form, {
      id: acc.id,
   ...
   props.onClose()
   consts.notyf.error(`Unable to find account with ID:${props.id} `)
}
